...
function add_drive () {
  ...
  mkfs.vfat "$filename" -s 8 -F 32 -n "$label"
...
then
  ...
  echo "options g_mass_storage file=$MUSIC_DISK_FILE_NAME,$CAM_DISK_FILE_NAME removable=1,1 ro=0,0 stall=0 iSerialNumber=123456,123457" > "$G_MASS_STORAGE_CONF_FILE_NAME"
else
  echo "options g_mass_storage file=$CAM_DISK_FILE_NAME removable=1 ro=0 stall=0 iSerialNumber=123457" > "$G_MASS_STORAGE_CONF_FILE_NAME"
fi
